...
//  Puradom
//
//  Created by Lakeba_26 on 23/04/18.
//  Copyright © 2018 Lakeba_26. All rights reserved.
//

import UIKit

class SearchProductViewController: UIViewController,internetConnectionDelegate {
   
    @IBOutlet var lbl_norecords: UILabel!
    ...
    func loadProductList()
    {
        if(Utilities.checkForInternet())
        {
            ...
            Alamofire.request(PRODUCT_LIST+product_ID).responseJSON { response in
                ...
                if let json = response.result.value {
                    if((((json as AnyObject).value(forKey: "fields")! as! NSNumber).intValue) == 1)
                        {
                            self.lbl_norecords.isHidden = true
